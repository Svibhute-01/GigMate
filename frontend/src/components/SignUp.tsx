  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (role === "student") {
        ...
        payload.portfolioLinks = form.portfolioLinks
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
      } else {
      ...
      <div className="w-full max-w-[520px] rounded-2xl border border-[#0f3d34] bg-gradient-to-b from-[#062a23] to-[#031c17] p-6 shadow-xl">
        ...
        {step === "role" ? (
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <p className="text-gray-400 text-xs mb-2">As {role === "student" ? "Student Freelancer" : "Client"}</p>
            ...
            {role === "student" ? (
              <>
                <Field label="College" value={form.college} onChange={(v) => update("college", v)} />
                <Field label="Skills (comma separated)" value={form.skills} onChange={(v) => update("skills", v)} />
                <Field label="Portfolio links (comma separated)" value={form.portfolioLinks} onChange={(v) => update("portfolioLinks", v)} />
